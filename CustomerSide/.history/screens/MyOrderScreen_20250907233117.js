...
const MyOrderScreen = () => {
    ...
    useEffect(() => {
        const fetchOrders = () => {
            ...
            // Create the query based on the current user and active tab
            const q = query(
            ...
            const unsubscribe = onSnapshot(q, (querySnapshot) => {
                const fetchedOrders = querySnapshot.docs.map(doc => {
                    const data = doc.data();
                    // Ensure the createdAt field is a Date object for formatting
        ...
        // Rerun the fetch function whenever the activeTab changes
        setIsLoading(true);
        fetchOrders();
    }, [activeTab]);
