...
export default function OrderConfirmationScreen() {
    ...
    const { orderId } = route.params || {}; // Get orderId passed from CheckoutScreen
    ...
    useEffect(() => {
        const loadOrder = async () => {
            ...
            try {
                ...
                // Fetch the order from Firestore using orderId
                const snap = await getDoc(doc(db, 'orders', orderId));
                if (snap.exists()) {
                    setOrder({ id: snap.id, ...snap.data() });
                } else {
                    setOrder(null); // No order found for the given orderId
                }
            } catch (err) {
                console.error('Failed to fetch order:', err);
                setOrder(null); // Handle error
            } finally {
                setLoadingOrder(false);
            }
        };
        ...
        loadOrder();
    }, [orderId]); // Reload if orderId changes (relevant when navigating from Checkout)

    // Show loading indicator while data is being fetched or fonts aren't loaded
    ...
    // If there's an order loaded, show real details. Otherwise show fallback static content.
    const itemsList = order?.items || [{ name: 'Floor Standing Mirror', quantity: 1 }];
    const total = order?.total || 2000;
    const address = order?.address || '123 Tonying Street, Mactan Proper, Lapu-Lapu City';
    const payment = order?.paymentMethod || 'Bank Card';
        ...
        <View style={styles.container}>
            ...
            <View style={styles.content}>
                ...
                <View style={styles.sectionContainer}>
                    ...
                    {/* Show the first item and the rest count */}
