        ...
        const fetchOrders = () => {
            ...
            const unsubscribe = onSnapshot(q, (querySnapshot) => {
                ...
                querySnapshot.forEach((doc) => {
                    ...
                    // Get the `order1` sub-collection for the current order
                    const order1Ref = collection(db, `orders/${orderId}/order1`);

                    // Real-time listener for the `order1` sub-collection
                    const order1Unsubscribe = onSnapshot(order1Ref, (order1Snapshot) => {
                        order1Snapshot.forEach((order1Doc) => {
                            ...
                            fetchedOrders.push({
                                ...
                                items: order1Data.items || [],  // Ensure items is an array of objects
                                status: order1Data.status || 'Pending',  // Get status from the first document
                                total: order1Data.total || 0,  // Get total from the first document
                    ...
                    return () => order1Unsubscribe(); // Cleanup listener when component unmounts
                });
            });
