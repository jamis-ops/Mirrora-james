...
export default function HomeScreen() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const bannerList = bannerSnapshot.docs.map((d) => ({ id: d.id, ...d.data() }));
  ...
  const addToWishlist = async (product) => {
    ...
    const ref = doc(db, "wishlists", user.uid, "items", product.id);
  ...
  return (
    <View style={styles.container}>
      <ScrollView>
        ...
        <View style={styles.searchContainer}>
          <View style={styles.searchInputWrapper}>
            ...
            <TextInput style={styles.searchInput} placeholder="Search here..." placeholderTextColor="#777" />
          </View>
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <ImageBackground source={{ uri: item.imageUrl }} style={styles.banner} imageStyle={styles.bannerImageStyle}>
        ...
        <View style={styles.sectionHeader}>
          <TouchableOpacity style={styles.categoryDropdownButton} onPress={() => setDropdownVisible(!isDropdownVisible)}>
            <Text style={styles.sectionTitle}>{selectedCategory}</Text>
            <Icon name={isDropdownVisible ? "chevron-up" : "chevron-down"} size={20} color="#000" style={{ marginLeft: 5 }} />
          </TouchableOpacity>
            ...
            renderItem={({ item }) => {
              ...
              return (
                <TouchableOpacity
                  ...
                  onPress={() => navigation.navigate("ProductScreen", { product: item, addToCart: addToCart, addToWishlist: addToWishlist })}
                >
                  <View style={styles.productImageContainer}>
                    <Image source={{ uri: item.imageUrl }} style={styles.productImage} />
                    <TouchableOpacity style={styles.heartIcon} onPress={() => addToWishlist(item)}>
                      <Icon name={isTempClicked ? "heart" : "heart-outline"} size={20} color={isTempClicked ? "red" : "#fff"} />
                    </TouchableOpacity>
                  </View>
      ...
      {/* Dropdown */}
      <CategoriesDropdown isVisible={isDropdownVisible} onClose={() => setDropdownVisible(false)} onSelectCategory={setSelectedCategory} />
...
const styles = StyleSheet.create({
  ...
  searchContainer: { flexDirection: "row", alignItems: "center", paddingHorizontal: 20, marginTop: 20 },
  searchInputWrapper: { flexDirection: "row", alignItems: "center", flex: 1, backgroundColor: "#F3F4F6", borderRadius: 10, paddingHorizontal: 15 },
  ...
  banner: { width: 320, height: 150, marginRight: 15, borderRadius: 15, overflow: "hidden", justifyContent: "center", padding: 20, shadowColor: "#000", shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.1, shadowRadius: 5, elevation: 5 },
  ...
  customizeButton: { backgroundColor: "rgba(255, 255, 255, 0.7)", paddingVertical: 8, paddingHorizontal: 15, borderRadius: 20, marginTop: 10, alignSelf: "flex-start" },
  customizeButtonText: { fontFamily: "Montserrat_600SemiBold", fontSize: 12, color: "#A68B69" },
  ...
  productCard: { width: "47%", backgroundColor: "#F9F9F9", borderRadius: 10, overflow: "hidden", marginBottom: 20, shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 3, elevation: 3 },
  productImageContainer: { position: "relative" },
  productImage: { width: "100%", height: 200, resizeMode: "cover" },
  heartIcon: { position: "absolute", top: 10, right: 10, backgroundColor: "rgba(0, 0, 0, 0.3)", borderRadius: 15, padding: 5 },
  productInfo: { padding: 10 },
  productName: { fontFamily: "Montserrat_600SemiBold", fontSize: 14 },
  priceAndButton: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginTop: 10 },
  ...
  dropdownContainer: { position: "absolute", top: 250, left: 20, width: 150, backgroundColor: "#fff", borderRadius: 10, shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 3, elevation: 5, paddingVertical: 5 },
