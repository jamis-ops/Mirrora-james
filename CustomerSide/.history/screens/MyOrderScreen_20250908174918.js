...
const MyOrderScreen = () => {
    ...
    useEffect(() => {
        const fetchOrders = () => {
            ...
            // Create a query to fetch orders for the current user and order by createdAt (newest first)
            const q = query(
                collection(db, 'orders'),
                where('userId', '==', user.uid), // Ensure that userId matches correctly
            ...
            const unsubscribe = onSnapshot(q, (querySnapshot) => {
                ...
                const fetchedOrders = querySnapshot.docs.map(doc => {
                    const data = doc.data();
                    const createdAt = data.createdAt ? data.createdAt.toDate() : new Date(); // Convert Firestore Timestamp to Date
                    return {
                        id: doc.id,
                        ...data,
                        createdAt: createdAt, // Make sure createdAt is a JavaScript Date
                    };
                });
        ...
        setIsLoading(true);
        fetchOrders();
    }, [activeTab]);  // Re-run the fetch when activeTab changes
                                ...
                                <View key={index} style={styles.orderBody}>
                                    ...
                                    <View style={styles.productDetails}>
                                        ...
                                        <View style={styles.paymentInfo}>
                                            ...
                                            <Text style={styles.paymentAmount}>₱ {Number(order.total).toLocaleString('en-US')}</Text>
                                        </View>
                                    </View>
                                </View>
                            ))}
