        ...
        <svg width={size} height={size} className="transform -rotate-90">
          {segments.map((segment, index) => (
            <circle
              ...
              strokeDashoffset={-cumulativeAngle}
              transform={`rotate(${segment.startAngle} ${radius} ${radius})`}
  ...
  return (
    <div className="relative h-64">
      ...
      <div className="absolute bottom-0 left-0 right-0 flex justify-between px-2">
        {data.map((item, index) => (
          <div key={index} className="text-center text-xs text-gray-600">{item.name}</div>
        ))}
      </div>
    </div>
  );
...
const DateFilter = ({ timeFrame, onTimeFrameChange }) => {
  const timeFrames = [
    ...
    const fetchData = async () => {
      try {
        ...
        const [orders, revenue, products, reviews] = await Promise.all([
          fetchOrders(filters), fetchTotalRevenue(filters), fetchProducts(filters), fetchReviews()
        ]);
        ...
        setDashboardData({
          ...
          todaysSales: `₱${(revenue * 0.1).toLocaleString()}`,
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-12">
            <div className="lg:col-span-2 bg-white/60 backdrop-blur-lg rounded-3xl p-8 border border-gray-200/50 shadow-xl shadow-black/5 relative">
              ...
              <div className="flex items-center justify-between mb-8">
                ...
                <DateFilter timeFrame={timeFrame} onTimeFrameChange={handleTimeFrameChange} />
              </div>
            ...
            <div className="space-y-8">
              <div className="bg-gradient-to-br from-[#A68B69] via-[#8a6e51] to-[#6d5940] text-white p-8 rounded-3xl shadow-2xl shadow-[#A68B69]/20 relative overflow-hidden">
                <div className="relative z-10">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-[#E0DAD6] text-sm font-medium mb-2">Today's Sales</p>
              ...
              <div className="bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500 text-white p-8 rounded-3xl shadow-2xl shadow-indigo-500/20 relative overflow-hidden">
                <div className="relative z-10">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-indigo-100 text-sm font-medium mb-2">Satisfaction</p>
